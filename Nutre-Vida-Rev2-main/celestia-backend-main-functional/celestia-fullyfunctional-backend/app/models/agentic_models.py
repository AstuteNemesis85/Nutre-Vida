...
class ConversationMemory(Base):
    ...
    content = Column(Text)
    context_data = Column(JSON, default=dict)  # Store meal context, user state, etc.
    embedding_vector = Column(JSON, default=dict)  # For semantic search (future enhancement)
...
class HealthAlert(Base):
    ...
    title = Column(String)
    message = Column(Text)
    data_context = Column(JSON, default=dict)  # Supporting data for the alert
...
class SmartNotification(Base):
    ...
    recurrence_pattern = Column(JSON, default=dict)  # For recurring notifications
    personalization_data = Column(JSON, default=dict)  # User-specific customization
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class MealPlan(Base):
    ...
    goals = Column(JSON, default=dict)  # Nutritional and health goals for this plan
    preferences = Column(JSON, default=dict)  # User preferences considered
    plan_data = Column(JSON, default=dict)  # Complete meal plan structure
...
class MealPlanItem(Base):
    ...
    food_items = Column(JSON, default=list)  # List of food items with quantities
    nutritional_info = Column(JSON, default=dict)  # Calculated nutrition
    preparation_notes = Column(Text)
    alternatives = Column(JSON, default=list)  # Alternative food options
...
class UserBehaviorPattern(Base):
    ...
    pattern_data = Column(JSON, default=dict)  # Detailed pattern information
...
class PredictiveInsight(Base):
    ...
    prediction_data = Column(JSON, default=dict)  # Supporting data and probabilities
    confidence_level = Column(Float, default=0.0)  # 0.0 to 1.0
    time_horizon = Column(String)  # 'short_term', 'medium_term', 'long_term'
    actionable_recommendations = Column(JSON, default=list)
    is_active = Column(Boolean, default=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
