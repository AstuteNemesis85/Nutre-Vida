...
class SchedulerService:
    ...
    def _send_daily_summaries(self):
        ...
        try:
            ...
            # Get users who want daily summaries (filter in Python for SQLite compatibility)
            all_verified = db.query(User).filter(User.phone_verified == True).all()
            users = [
                u for u in all_verified
                if isinstance(u.notification_preferences, dict)
                and u.notification_preferences.get('daily_summary', False)
            ]
    ...
    def _send_weekly_summaries(self):
        ...
        try:
            ...
            # Get users who want weekly summaries (filter in Python for SQLite compatibility)
            all_verified = db.query(User).filter(User.phone_verified == True).all()
            users = [
                u for u in all_verified
                if isinstance(u.notification_preferences, dict)
                and u.notification_preferences.get('weekly_summary', False)
            ]
    ...
    def _check_monthly_summaries(self):
        ...
        try:
            ...
            # Get users who want monthly summaries (filter in Python for SQLite compatibility)
            all_verified = db.query(User).filter(User.phone_verified == True).all()
            users = [
                u for u in all_verified
                if isinstance(u.notification_preferences, dict)
                and u.notification_preferences.get('monthly_summary', False)
            ]
