...
meal_planner_model = genai.GenerativeModel("models/gemini-2.5-flash")
...
class IntelligentMealPlanner:
    ...
    def generate_meal_plan(
        ...
        try:
            ...
            # Determine plan goals - always start with defaults, then merge user-specific goals
            plan_goals = self._determine_plan_goals(user, user_context)
            if specific_goals:
                # Merge frontend goals into determined goals
                if specific_goals.get('calorie_target'):
                    plan_goals['target_calories'] = int(specific_goals['calorie_target'])
                if specific_goals.get('dietary_restrictions'):
                    plan_goals['dietary_restrictions'] = specific_goals['dietary_restrictions']
                if specific_goals.get('cuisine_preference'):
                    plan_goals['cuisine_preference'] = specific_goals['cuisine_preference']
                if specific_goals.get('meals_per_day'):
                    plan_goals['meal_frequency'] = int(specific_goals['meals_per_day'])
                if specific_goals.get('budget'):
                    plan_goals['budget'] = specific_goals['budget']
                # Also accept target_calories directly if provided
                if specific_goals.get('target_calories'):
                    plan_goals['target_calories'] = int(specific_goals['target_calories'])
