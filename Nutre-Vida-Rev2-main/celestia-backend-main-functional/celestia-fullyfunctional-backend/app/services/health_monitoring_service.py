...
class HealthMonitoringService:
    ...
    def _monitor_nutrition_patterns(
        ...
        # Calculate average daily nutrition (guard against empty filtered lists)
        cal_vals = [s.total_calories for s in daily_summaries if s.total_calories and s.total_calories > 0]
        prot_vals = [s.total_protein for s in daily_summaries if s.total_protein and s.total_protein > 0]
        carb_vals = [s.total_carbs for s in daily_summaries if s.total_carbs and s.total_carbs > 0]
        fat_vals = [s.total_fat for s in daily_summaries if s.total_fat and s.total_fat > 0]
        
        avg_calories = statistics.mean(cal_vals) if cal_vals else 0
        avg_protein = statistics.mean(prot_vals) if prot_vals else 0
        avg_carbs = statistics.mean(carb_vals) if carb_vals else 0
        avg_fat = statistics.mean(fat_vals) if fat_vals else 0
    ...
    def _predict_health_risks(self, daily_summaries: List[DailySummary]) -> Dict[str, Any]:
        ...
        # Calculate averages (guard against empty filtered lists)
        cal_vals = [s.total_calories for s in daily_summaries if s.total_calories and s.total_calories > 0]
        carb_vals = [s.total_carbs for s in daily_summaries if s.total_carbs and s.total_carbs > 0]
        fat_vals = [s.total_fat for s in daily_summaries if s.total_fat and s.total_fat > 0]
