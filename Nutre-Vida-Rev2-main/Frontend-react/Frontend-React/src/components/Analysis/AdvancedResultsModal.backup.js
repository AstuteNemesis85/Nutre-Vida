        ...
        return (
          <>
            ...
            {data && data.map((swap, index) => (
              <SwapItem key={index}>
                <SwapInfo>
                  <SwapOriginal>Currently: {swap.name || swap.original}</SwapOriginal>
                  <SwapNew>Alternative: {swap.quantity ? `${swap.quantity} of ` : ''}{swap.alternative}</SwapNew>
                  <SwapBenefits>
                    {swap.benefits && (
                      <div>
                        <strong>Benefits:</strong> {swap.benefits}
                      </div>
                    )}
                    {swap.indianBenefit && (
                      <div style={{ marginTop: '8px' }}>
                        <strong>Cultural Context:</strong> {swap.indianBenefit}
                      </div>
                    )}
                    {(swap.calories || swap.protein || swap.carbs || swap.fat) && (
                      <div style={{ marginTop: '8px', fontSize: '0.9em', color: 'rgba(255, 255, 255, 0.7)' }}>
                        Nutrition per serving:
                        {swap.calories && ` ${swap.calories} cal`}
                        {swap.protein && ` | ${swap.protein}g protein`}
                        {swap.carbs && ` | ${swap.carbs}g carbs`}
                        {swap.fat && ` | ${swap.fat}g fat`}
                      </div>
                    )}
                  </SwapBenefits>
                </SwapInfo>
