...
const FoodAnalysis = () => {
  ...
  const uploadAndAnalyzeImage = async () => {
    ...
    try {
      ...
      if (!currentSessionId) {
        currentSessionId = await initSession(true);
  ...
  const exportPDF = () => {
    ...
    try {
      const doc = new jsPDF();
      const data = results.analysis_data;
      ...
      doc.text('Nutrition Analysis Report', pageWidth / 2, y, { align: 'center' });
      y += 10;
      ...
      const totals = [
        `Calories: ${data.total_calories ?? 'N/A'}`,
        `Protein: ${data.total_protein ?? 'N/A'}g`,
        `Carbs: ${data.total_carbs ?? 'N/A'}g`,
        `Fat: ${data.total_fat ?? 'N/A'}g`
      ];
      doc.text(totals.join('    |    '), 20, y);
      y += 20;
      ...
      if (data.items && data.items.length > 0) {
        ...
        doc.text('Item', cols[0], y + 7);
        doc.text('Calories', cols[1], y + 7);
        doc.text('Protein (g)', cols[2], y + 7);
        doc.text('Carbs (g)', cols[3], y + 7);
        doc.text('Fat (g)', cols[4], y + 7);
        y += 12;
        ...
        data.items.forEach((item, index) => {
          ...
          doc.setFontSize(10);
          doc.setTextColor(60, 60, 60);
          const name = (item.name || 'Unknown').substring(0, 25);
          doc.text(name, cols[0], y + 5);
          doc.text(String(item.calories ?? '-'), cols[1], y + 5);
          doc.text(String(item.protein ?? '-'), cols[2], y + 5);
          doc.text(String(item.carbs ?? '-'), cols[3], y + 5);
          doc.text(String(item.fat ?? '-'), cols[4], y + 5);
          y += 10;
        });
        y += 5;
      }
      ...
      if (data.health_score != null) {
        ...
        doc.text(`Health Score: ${data.health_score}/10`, 20, y);
        y += 10;
      }
      ...
      doc.text('Generated by Nutre Vida - Your AI Nutrition Assistant', pageWidth / 2, 285, { align: 'center' });
