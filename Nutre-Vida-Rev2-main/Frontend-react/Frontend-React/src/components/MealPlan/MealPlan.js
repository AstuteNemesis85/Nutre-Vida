...
const MealPlan = () => {
  ...
  const generateMealPlan = async () => {
    ...
    try {
      ...
      if (data.meal_plan) {
        // Ensure the meal_plan has a 'days' property
        let plan = data.meal_plan;
        if (typeof plan === 'string') {
          plan = parseMealPlanText(plan);
        }
        // If plan has days array, use it directly; otherwise try to extract
        if (!plan.days && plan.meal_plan_data) {
          plan = plan.meal_plan_data;
        }
        if (!plan.days) {
          // Wrap raw object in a days structure if possible
          plan = { days: Array.isArray(plan) ? plan : [] };
        }
        setMealPlan(plan);
        showToast("Intelligent meal plan generated successfully!", 'success');
      } else if (data.meal_plan_data) {
        // Handle direct meal_plan_data response
        let plan = data.meal_plan_data;
        if (typeof plan === 'string') {
          plan = parseMealPlanText(plan);
        }
        if (!plan.days) {
  ...
  const parseMealPlanText = (planText) => {
    // Try parsing as JSON first (in case Gemini returned JSON in a string)
    try {
      const jsonPlan = JSON.parse(planText);
      if (jsonPlan.days) return jsonPlan;
    } catch (e) {
      // Not JSON, continue with text parsing
    }

    // Enhanced parser for Gemini's markdown-formatted meal plans
    const days = [];
    ...
    lines.forEach(line => {
      ...
      const trimmedLine = line.trim()
        .replace(/^#{1,4}\s*/, '')  // Remove markdown headers
        .replace(/^\*{1,2}\s*/, '') // Remove bold/italic markers at start
        .replace(/\*{1,2}$/,  '')   // Remove bold/italic markers at end
        .replace(/^\*\*(.+?)\*\*:?/, '$1:') // Convert **text**: to text:
        .replace(/^[-•]\s*/, '')    // Remove bullet points
        .trim();
      
      if (!trimmedLine) return;
      
      // Check if it's a day header (Day 1, Monday, ### Day 1, etc.)
      if (trimmedLine.match(/^(Day\s*\d+|Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)/i)) {
        if (currentDay) days.push(currentDay);
        currentDay = {
          day: trimmedLine.replace(/:$/, ''),
          meals: {}
        };
        currentMeal = null;
      }
      // Check if it's a meal header (Breakfast, Lunch, etc.)
      else if (trimmedLine.match(/^(Breakfast|Lunch|Dinner|Snacks?|Evening\s*Snack|Morning\s*Snack|Mid-Morning|Brunch)/i)) {
        const mealType = trimmedLine.toLowerCase()
          .replace(/evening\s*snack/i, 'snack')
          .replace(/morning\s*snack/i, 'snack')
          .replace(/mid-morning/i, 'snack')
          .replace(/:$/, '');
      ...
      // It's a meal item
      else if (currentDay && currentMeal && trimmedLine.length > 2) {
        // Extract calories if present (various formats)
        const calorieMatch = trimmedLine.match(/[~≈]?\s*(\d+)\s*(?:cal(?:ories)?|kcal)/i);
        const calories = calorieMatch ? calorieMatch[1] : null;
        let name = trimmedLine
          .replace(/[~≈]?\s*\d+\s*(?:cal(?:ories)?|kcal)\s*/gi, '')
          .replace(/\([\s,]*\)/g, '') // Remove empty parentheses
          .replace(/^\s*[-•*]\s*/, '') // Remove leading bullets
          .replace(/\s*\|\s*/g, ', ')  // Convert pipe separators
          .trim();
